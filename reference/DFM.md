...
- save.full.state:
  ...
  `ss_full`? Set to `FALSE` to reduce object size.
...
- `ss_full`:
  ...
  list of full state-space matrices and full-state smoothing results
  used internally (`A`, `C`, `Q`, `R`, `F_0`, `P_0`, `F_smooth`,
  `P_smooth`). Only stored when `save.full.state = TRUE`.
...
[`SKFS`](https://docs.ropensci.org/dfms/reference/SKFS.md) for a
complete code example.

When modeling observations errors as AR(1) processes (`idio.ar1 = TRUE`)
and/or when quarterly variables are included
(`quarterly.vars = c(...)`), the state-space form of the model is
augmented in order to estimate the additional parameters and apply
appropriate restrictions - see the [theoretical
vignette](https://raw.githubusercontent.com/ropensci/dfms/main/vignettes/dynamic_factor_models_paper.pdf)
for details. But `DFM()` returns matrices for the classical form of the
factor model, and thus these modifications do not affect the output
format.
