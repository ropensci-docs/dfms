...
- use.full.state:
  ...
  logical. Use the full state-space (if available) when computing
  residuals for optional residual forecasting. When `idio.ar1 = TRUE`,
  this yields residuals after both factor and idiosyncratic components;
  set to `FALSE` to use factor-only residuals. Falls back to the compact
  form if unavailable or if `method = "pca"`.
