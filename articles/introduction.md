...
Modugno (2014)[^1], henceforth BM14, and covers the Euro Area from
...
![](https://raw.githubusercontent.com/ropensci/dfms/main/vignettes/figure/unnamed-chunk-2-1.png)

The data frame `BM14_Models` provides information about the series[^2],
and the various models estimated by BM14.
...
stationary. Data is also scaled and centered[^3] in the main
[`DFM()`](https://docs.ropensci.org/dfms/reference/DFM.md) function,
...
![](https://raw.githubusercontent.com/ropensci/dfms/main/vignettes/figure/unnamed-chunk-4-1.png)

## Determining the Structure of the Model

Before estimating a model, the
[`ICr()`](https://docs.ropensci.org/dfms/reference/ICr.md) function can
be applied to determine the number of factors. It computes 3 information
criteria proposed in Bai and NG (2002)[^4], whereby the second criteria
generally suggests the most parsimonious model.
...
![](https://raw.githubusercontent.com/ropensci/dfms/main/vignettes/figure/unnamed-chunk-5-1.png)
...
kink was suggested by Onatski (2010)[^5], but this is currently not
implemented in
...
![](https://raw.githubusercontent.com/ropensci/dfms/main/vignettes/figure/unnamed-chunk-6-1.png)
...
The selection thus suggests we should estimate a factor model with r = 4
factors and p = 3 lags[^6]. Before estimating the model I note that
*dfms* does not deal with seasonality in series, thus it is recommended
to also seasonally adjust data, e.g. using the *seasonal* package before
estimation. BM14 only use seasonally adjusted series, thus this is not
necessary with the example data provided.

## Estimation and Exploration

Estimation can then simply be done using the
[`DFM()`](https://docs.ropensci.org/dfms/reference/DFM.md) function with
parameters r and p[^7].
...
![](https://raw.githubusercontent.com/ropensci/dfms/main/vignettes/figure/unnamed-chunk-8-1.png)
...
![](https://raw.githubusercontent.com/ropensci/dfms/main/vignettes/figure/unnamed-chunk-10-1.png)
...
![](https://raw.githubusercontent.com/ropensci/dfms/main/vignettes/figure/unnamed-chunk-10-2.png)
...
estimates following Doz, Giannone and Reichlin (2011)[^8], where the
Kalman Filter and Smoother is run only once. Both estimates are also
computed by [`DFM()`](https://docs.ropensci.org/dfms/reference/DFM.md)
...
![](https://raw.githubusercontent.com/ropensci/dfms/main/vignettes/figure/unnamed-chunk-11-1.png)
...
autoregressive model, the forecasts tend to zero quite quickly[^9].
...
![](https://raw.githubusercontent.com/ropensci/dfms/main/vignettes/figure/unnamed-chunk-14-1.png)
...
![](https://raw.githubusercontent.com/ropensci/dfms/main/vignettes/figure/MQ-1.png)
...
This option can be combined with mixed-frequency estimation. Since
estimating AR(1) errors increases runtime, the examples below use the
...
coefficients are small, indicating little serial correlation in the
...
![](https://raw.githubusercontent.com/ropensci/dfms/main/vignettes/figure/unnamed-chunk-17-1.png)

In general, Doz et al. (2011, 2012) show that in the presence of serial
correlation, the factors are still consistently estimated as n, T \to
\infty using an exact DFM specification. Thus modeling serial
correlation is particularly important in smaller samples. Banbura &
Modugno (2014) also show that modeling the serial correlation improves
forecast accuracy, but only at short horizons.

## News Decomposition

The [`news()`](https://docs.ropensci.org/dfms/reference/news.md)
function decomposes forecast revisions into contributions from new data
releases following Banbura and Modugno (2014). The idea is to compare an
old vintage (with ragged-edge missing values) to a new vintage where
some missing values are now observed. The output reports the forecast
revision for a target series (usually a quarterly variable such as GDP
not yet observed in the current period) and decomposes the change in its
DFM-based forecast into series contributions.

Below, we use a medium-size mixed-frequency dataset, create a previous
vintage, and compute news impacts for the quarterly targets.
...
Apart from the overall forecast revision, the
[`news()`](https://docs.ropensci.org/dfms/reference/news.md) function
measures, for each series, the impact on the revision, which is a
function of the news (innovation) multiplied by the gain (weight). The
formula `impact` = `news` \times `gain` always holds, and `news` =
`actual` - `forecast` holds if there is only one change to the series
between vintages.
...
[`.VAR()`](https://docs.ropensci.org/dfms/reference/dot-VAR.md), or
Kalman Filtering and Smoothing with
[`SKFS()`](https://docs.ropensci.org/dfms/reference/SKFS.md), or
separately with
[`SKF()`](https://docs.ropensci.org/dfms/reference/SKF.md) followed by
[`FIS()`](https://docs.ropensci.org/dfms/reference/FIS.md). To my
...
[`ainv()`](https://docs.ropensci.org/dfms/reference/ainv.md) and
[`apinv()`](https://docs.ropensci.org/dfms/reference/ainv.md). These are
often faster than [`solve()`](https://rdrr.io/r/base/solve.html), and
somewhat more robust in near-singularity cases.
...
vignette](https://raw.githubusercontent.com/ropensci/dfms/main/vignettes/dynamic_factor_models_paper.pdf).
...
[^1]: Banbura, M., & Modugno, M. (2014). Maximum likelihood estimation
...
[^5]: Onatski, A. (2010). Determining the number of factors from
...
[^6]: Some authors like Doz, Giannone, and Reichlin (2012) additionally
    ...
    factors’ and given an extra parameter q, to be less than the number
    of ‘static factors’ r. I find this terminology confusing and the
    feature not very useful for most practical purposes, thus I have not
    implemented it in *dfms*.
...
[^7]: Users can also choose from two different implementations of the EM
    ...
    implementation of Doz, Giannone and Reichlin (2012), henceforth
    ...
    edges at the beginning or end of the sample or series at different
    frequencies. With complete datasets, both `em.method = "DGR"` and
    `em.method = "BM"` give identical factor estimates, and in this case
    `em.method = "DGR"` should be used.
...
[^9]: Depending also on the lag-order of the factor-VAR. Higher
    lag-orders produce more interesting forecast dynamics.
